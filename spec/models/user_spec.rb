...
RSpec.describe User, type: :model do
  it { is_expected.to have_many(:karma_points) }
  ...
  describe '#valid?' do
    ...
    it { is_expected.to ensure_length_of(:username).is_at_least(2).is_at_most(32) }
  ...
  describe '.by_karma' do
    it 'returns users in order of highest-to-lowest karma' do
      ...
      expect(User.by_karma).to eq [user_high, user_med, user_low]
    end
  end
  ...
  describe '#total_karma' do
    ...
    it 'returns the total karma for the user' do
      expect(user.total_karma).to eq 500
    end
  end
